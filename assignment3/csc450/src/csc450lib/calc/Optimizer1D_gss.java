...
public class Optimizer1D_gss extends Optimizer1D {
    ...
    public OptimizerRecord1D minimize(float xMin, float xMax, float tol, int maxIters) {
        ...
        // System.out.println("xMin: " + xMin + " | xMax: " + xMax + " | midX: " +
        // ((xMax - xMin) / 2));
    ...
    public float[][] minimizeVisualization(float xMin, float xMax, float tol, int maxIters) {
        Function1D f = getFunction();
        int numIters = 1;
        ...
        while ((xMax - xMin) > tol && numIters < maxIters) {
            ...
            float midY = f.func(midX);
            float distance = Math.abs(midY - midX);
            points.add(new float[] { midX, midY, distance });
        }
